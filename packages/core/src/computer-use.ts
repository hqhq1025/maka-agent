...
export function computerUseApprovalSummary(args: unknown): ComputerUseApprovalSummary {
  ...
  const action = typeof rawAction === 'string' && APPROVAL_ACTIONS.has(rawAction)
    ? rawAction
    : 'unknown';
  ...
  const app = typeof rawApp === 'string' ? bounded(rawApp, 256) : undefined;
  const windowId = typeof rawWindowId === 'number' && Number.isInteger(rawWindowId)
    ? rawWindowId
    : undefined;
  const observationId = typeof rawObservationId === 'string'
    ? stableIdentifier(rawObservationId)
    : undefined;
  const rememberForTurnAllowed =
    approvalClass === 'metadata_read'
      ? action === 'list_apps' || app !== undefined || windowId !== undefined
      : observationId !== undefined || app !== undefined || windowId !== undefined;
...
export function computerUseApprovalScopeKey(args: unknown): string {
  const summary = computerUseApprovalSummary(args);
  return `computer_use:${JSON.stringify([
    summary.approvalClass,
    summary.action,
    summary.app ?? null,
    summary.windowId ?? null,
