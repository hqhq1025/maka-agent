...
export function createCuaDriverBackend(opts: CuaDriverBackendOptions): CuDispatchBackend & {
  ...
  return {
    ...
    async runSemantic(action: CuSemanticAction, signal, context) {
      // The screen being locked is not the user competing for the pointer: it
      // is the user gone. An element action is refused for the same reason a
      // synthesized one is — nobody is there to see what it did.
      const locked = await screenLockFailure();
