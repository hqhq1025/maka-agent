...
describe('resolveCuaDisplaySnapshots', () => {
  it('maps a 2x primary display to screenshot pixels', () => {
    assert.deepEqual(resolveCuaDisplaySnapshots({
  ...
  it('keeps only a proven primary screenshot when mixed-scale atlas mapping is unknown', () => {
    ...
    assert.deepEqual(snapshots.map((display) => display.displayId), ['main']);
  });

  it('fails closed when screenshot dimensions match neither full atlas nor primary display', () => {
    assert.deepEqual(resolveCuaDisplaySnapshots({
