...
export function buildComputerUseTools(deps: {
  ...
  const tool: MakaTool<ComputerParams, ComputerToolResult> = {
    ...
    description:
      ...
      + 'for semantic element actions, safe text value updates, wait, zoom, or another observation. Every successful mutating action returns a fresh screenshot when available '
      ...
      + 'Coordinate click, scroll, drag, press_key, and pixel fallback are currently disabled because the compatibility event backend can interfere '
      + 'with the user\'s physical input; these actions fail closed with unsupported_action. Do not retry them or assume they succeeded. '
      ...
      + 'Electron text uses a uniquely resolved page target plus read-back verification. '
