...
No third-party executable ships in the artifact. Maka's Computer Use executor is
`maka-cu`, built from Maka's own source by `scripts/prepare-maka-cu.mjs` and
pinned by digest in `apps/desktop/bundled-tools.json`. It is not signed, so it
is not distributed at all yet — `scripts/verify-macos-arm64-dmg.mjs` forbids its
